...
doctorOfficeName = isNotNull(doctorOfficeName)
  ? decodeURIComponent(doctorOfficeName)
  : "";
var retId = getUrlParam("retid");
var isShowDbjzk = false;
var delayRegister = false;
var isApponitMemberList = [];
var maxApponitmember = 5;
$(function () {
        ...
        if (d.Code == 10000) {
          if (d.Result.value == 2) {
            ...
            if (hospitalId == 1565) {
              ...
              hintContent =
                "很抱歉，您不符合门诊预约条件，请您到发热门诊就诊。感谢您的理解与配合。";
            } else if (hospitalId == 2134 || hospitalId == 1026278) {
              hintContent =
                "基于您的身体状况，我们暂时无法为您提供线上预约服务，您可以携带本人有效证件现场预约。";
            ...
            $("#surebtn").click(function () {
              if (isXiongzhanghao()) {
                location.href =
                  StrYyghUrl +
                  "/appoint/doctor/xzhDoctorIndex.html?deptId=" +
                  deptId +
                  "&doctorId=" +
                  doctorInfo.doctorUid +
                  "&platformType=" +
                  platformType;
              } else {
                location.href =
...
function LoadPage() {
  ...
  $("#reDateInfo").text(
    "（" +
      ...
      GetWeek(reDate) +
      " " +
      GetTimeName(arrangeInfo.timeid) +
      ")"
  );
...
function BindInputForm(lists) {
  ...
  var placeholder = "";
  var hasCardType =
    lists.filter(function (d) {
      return d.fiename.toLowerCase() == "cardtype";
    }).length > 0;
  $.each(lists, function (i, item) {
    var contorKey = item.fiename.toLowerCase();
    if (contorKey == "cardtype") hasCardType = true;
    if (contorKey == "checkcode") {
      checkcode = true;
      return;
    }
    ...
    if (plusFiename.indexOf("," + contorKey + ",") < 0) {
      ...
      if (notShowFiename.indexOf("," + contorKey + ",") < 0) {
        ...
        if (contorKey == "cardno") {
          if (!hasCardType) {
            con +=
              '<li><div class="pmn_tit c-333">证件类型：</div> <div class="pmn_con c-909090"><select id="cardtype" name="ghFormCon" MassTxt="证件类型必填" class="pminput" >';
            con +=
              '<option value="1">居民身份证</option><option value="3">中国护照</option><option value="7">台胞证</option><option value="8">国外护照</option><option value="9">港澳身份证</option><option value="10">婴幼儿出生证</option><option value="11">台湾身份证</option><option value="12">港澳台居住证</option>';
            con +=
              '</select><span class="seljt arrow-right"></span></div></li>';
          }
        ...
        switch (item.fietype) {
          ...
          default:
            switch (contorKey) {
              case "cliniccard": //就诊卡
                ...
                if (item.ismust != 1) {
                  clinicCardHtml +=
                    '<li id="cardliSel"><div class="pmn_tit c-333"><i class="iconfont\
                                 icon-zhifu\
                                 mr5"></i>就诊卡</div>\
                    ...
                    "</li>";
                  clinicCardHtml +=
                    '<li id="cardli" style="display:none;"><div class="pmn_tit c-333"><i class="iconfont icon-qiahao mr5"></i>' +
                    item.fienamec +
                    '</div><div class="pmn_con c-909090"><input class="pminput" type="text" id="' +
                    contorKey +
                    '" name="ghFormCon" maxlength="' +
                    maxlength +
                    '" placeholder="' +
                    placeholder +
                    '" /></div>' +
                  ...
                  clinicCardHtml +=
                    '<li id="cardliMust" ><div class="pmn_tit c-333"><i class="iconfont icon-qiahao mr5"></i>' +
                    item.fienamec +
                    '</div><div class="pmn_con c-909090"><input class="pminput" type="text" id="' +
                    contorKey +
                    '" name="ghFormCon" maxlength="' +
              ...
              default:
                con +=
                  '<div class="pmn_con c-909090"><input class="pminput" type="text" id="' +
                  contorKey +
                  '" name="ghFormCon" maxlength="' +
                  maxlength +
                  '" MassTxt="' +
          ...
          case "C": //下拉列表框
            if (contorKey == "selectarea") {
              con +=
                '<div class="pmn_con c-909090"><div class="pminput" id="' +
                contorKey +
                '" name="ghFormCon"' +
                ' MassTxt="' +
                item.parabsname +
                '"></div>';
              con +=
                '<span class="seljt arrow-right"></span>' +
                ShowNote(item) +
                "</div>";
            } else {
              if (item.dict) {
                con +=
                  '<div class="pmn_con c-909090"><select id="' +
                  contorKey +
                  '" name="ghFormCon" MassTxt="' +
                  item.parabsname +
                  '" class="pminput" >';
                ...
                con +=
                  '</select><span class="seljt arrow-right"></span>' +
                  ShowNote(item) +
                  "</div>";
              }
            }
          ...
          case "D": //日期选择框
            con +=
              '<div class="pmn_con c-909090"><input class="pminput" type="text" data-time="date" id="' +
              contorKey +
              '" name="ghFormCon" maxlength="' +
              maxlength +
              '" MassTxt="' +
          ...
          case "P": //阅读须知
            ...
            con +=
              '<input type="hidden" name="ghFormCon" id="' + contorKey + '" >';
            eval(js);
            break;
        }
        listHtml += con + "</li>";
  ...
  $(".input-group-radio").on("click", function () {
    $(this)
      .siblings(".input-group-radio")
      .find(".input-group-pack")
      .removeClass("checked")
      .find("input")
      .removeAttr("checked");
    $(this)
  ...
  function ShowNote(item) {
    if (item.shownote == "Y") {
      ...
      if (item.fiename == "ClinicCard" && isShowDbjzk) {
        urlDb =
          StrTradeUrl +
          "/trade/fabinfo/assistHosPage.html?channelId=" +
          channelId +
          "&hospitalId=" +
          hospitalId +
          "&busType=6";
        dbJzk =
          "&nbsp;&nbsp;<a href=" +
          urlDb +
          " class='card-link'>在线代办就诊卡&gt;&gt;</a>";
      }
      var noteHtml =
        '<div class="card-stp"><a href="javascript:;" onclick="$(this).parent().hide()"><i class="iconfont icon-guanbi"></i></a>' +
        item.parabsname +
        dbJzk +
        "</div>";
      return noteHtml;
...
function GetMemberList(first, selMem) {
  ...
  if (hospitalId == "1284" || (isDebug && hospitalId == "1314")) {
    ...
    var url =
      "addHosMember.html?platformType=" +
      platformType +
      "&hospitalId=" +
      hospitalId +
      "&from=" +
    ...
    success: function (json) {
      ...
      if (json) {
        ...
        if (json.Code == 10000) {
          var memberCount = 0;
          MemList = json.Result;
          isApponitMemberList = json.Result.filter(function (d) {
            return isNotNull(d.isApponit) && d.isApponit == 1;
          });
          var showMemberList =
            isApponitMemberList.length >= maxApponitmember
              ? isApponitMemberList
              : json.Result;
          $.each(showMemberList, function (i, item) {
...
function SelMem(item) {
  ...
  if ($("#telephon").length > 0 && item.Phone != null && item.Phone != "") {
    //发送验证码号码
      ...
      if (json) {
        if (json.Code == 10000) {
          if (
            ...
            json.Result[0].Cardno != null &&
            json.Result[0].Cardno != "初诊"
          ) {
  ...
  $.ajax({
    ...
    success: function (json) {
      ...
      if (json) {
        ...
        if (json.Code == 10000 && json.Result && json.Result.length > 0) {
          //有锁号并创建订单信息
          var orderInfo = json.Result[0];
          var ddOrderId = orderInfo.Orderid;
          //console.log(orderUid);
          if (
            ConfDialog("提示", "您有未处理的订单，请先处理", "查看订单", 10)
          ) {
            $("#surebtn").click(function () {
              window.location =
                "../orderRecord/recordDetail.html?platformType=" +
                platformType +
                "&orderid=" +
                ddOrderId;
              $("#openDiv").empty();
...
///立即预约
function SubOrder(flag) {
  // flag ==1 不实名验证 2不判断号源刷新
  myLayer.load("正在处理中");
  ...
  if ($("#cmb_disease").val() != "" && $("#cmb_diseasename").text() != "") {
    FormConKv.push({
      keyValue: $("#cmb_disease").val(),
      keyName: "cmb_disease",
    });
    FormConKv.push({
      keyValue: $("#cmb_diseasename").text(),
  ...
  if (isNotNull(DoctorRegOrder.selectArea)) {
    FormConKv.push({
      keyValue: DoctorRegOrder.selectArea,
      keyName: "selectArea",
    });
  }
  ...
  if (channelId == "9001181") {
    var glCustomChannel =
      $.cookie(channelId + "_attach") == null
        ? ""
        : $.cookie(channelId + "_attach");
  ...
  if (
    DoctorRegOrder.hospitalId == 31 &&
    isNotNull(DoctorRegOrder.cliniccard) &&
    DoctorRegOrder.cliniccard == 0
  ) {
    checkMemberInfoService.checkMemberRegwater();
  } else if (
    DoctorRegOrder.hospitalId == 33 &&
    isNotNull(DoctorRegOrder.cliniccard)
  ) {
    ...
    success: function (rejson) {
      ...
      if (rejson) {
        if (rejson.Code == 10000) {
          ...
          //                      }
          location.href =
            "confirmOrder.html?platformType=" +
            platformType +
            "&doctorSn=" +
            doctorSn +
            "&orderId=" +
        ...
        else if (rejson.Code == -771) {
          //未开放预约
          myLayer.clear();
          var showMsg =
            "预计放号时间：<span class='c-ff8100'>" + rejson.Data + "</span>";
          ConfDialog("未到放号时间", showMsg, "确定");
          $("#surebtn").click(function () {
            $("#openDiv").empty();
          });
        } else if (rejson.Code == -3004) {
          //信用值
          myLayer.clear();
          var url =
            StrUserUrl +
            "/UserCenter/web/member/credit.html?membersn=" +
            DoctorRegOrder.memberSn +
            "&hosid=" +
            OrderInfo.GH_HosID +
            "&platformType=" +
            PlatformType +
            "&sourcetype=0&sourceid=0";
          var showMsg =
            "您的信用不足，预约失败，点击<span style='color:red'>查看信用</span>可了解更多详情";
...
function SetJianHuRen() {
  ...
  try {
    if (
      (DoctorRegOrder.memberZhengjianid == null ||
        DoctorRegOrder.memberZhengjianid == "") &&
      GuardianID != null &&
      GuardianID != ""
    ) {
...
function VerOrderForm() {
  ...
  //收费类型
  DoctorRegOrder.regpaytype =
    $("#regpaytype").val() != null ? $("#regpaytype").val() : "";
  //就诊卡
  ...
  if (
    $("#selectarea").length > 0 &&
    isNotNull($("#selectarea").attr("data-gbcode"))
  ) {
  ...
  $.each(Ismust, function (i, item) {
    ...
    switch (itemKey) {
      ...
      case "familyaddress":
        if (
          !DoctorRegOrder["memberAddress"] ||
          DoctorRegOrder["memberAddress"] == "无"
        ) {
...
function UpdateMemberInfo() {
  ...
  $.ajax({
    ...
    error: function () {},
    success: function (json) {
      if (typeof AddPatientPool == "function") {
        //增加患者池
        AddPatientPool();
      }
    },
...
function ShowPayTypeMsg(type) {
  //
  if (hospitalId == "1284" && (type == "1" || type == "2")) {
    //中山附一
...
function toAddMember() {
  if (isApponitMemberList.length >= maxApponitmember) {
    ShowGh();
    myLayer.confirm({
      title: "提示",
      con:
    ...
    $.ajax({
      ...
      success: function (d) {
        if (d.Code == 10000) {
          ...
          d.Code == -13003 &&
          isNotNull(d.Data) &&
          isNotNull(d.Data.sfzh)
        ) {
          myLayer.clear();
          var sfzh = d.Data.sfzh;
          var msgHtml = "院内预留身份证：" + sfzh;
          msgHtml +=
            "<br/>身份证信息必须与院内预留身份证相同才能预约，请确认是否同步改为院内预留身份证（如院内预留身份证不正确请到医院修改后再预约）";
          myLayer.confirm({
            title: "提示",
            con: msgHtml,
            cancel: function () {
