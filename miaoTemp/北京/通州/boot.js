(function(g) {
    ...
    env.setCDNStaticServerUrl = function(url) {
        ...
        if (env.wxDebug && $env.getUrlParam("wxDebug") != "true") {
            env.appResourcesUrl = location.origin + "/";
            env.resourcesHome = location.origin + "/"
        }
    ...
    var getCookie = function(name) {
        try {
            var cookie = $env.hybrid ? env._cookie : document.cookie;
            if ($sys.isWeixin && $sys.isWeixin() && name == "WXApp" && env.isWeixinCookieOpen) {
                console.log("cookie open");
                return $encode(decodeURIComponent(window.WXApp))
            }
            if ($env.ngariMiniAppForWeb && name == "WXApp") {
                console.log("cookie open");
                return $encode(decodeURIComponent(window.WXApp))
            }
            var arg = name + "="
    ...
    env.getCookie = function(name) {
        if (env.isIOSZLB || $env.isZLBEntry) {
    ...
    env.checkIOSZlb = function() {
        var zlbFlag = env.getUrlParam("appkey") && (env.isZLB || env.getUrlParam("appkey").indexOf("zlbJT-") > -1 || env.getUrlParam("appkey").indexOf("zlb-") > -1) || env.getUrlParam("appkey") == "zjzwfw";
        return env.platform.name == "iOS" && (zlbFlag || env.isZLB)
    }
    ;
    env.isIOSZLB = env.checkIOSZlb();
    ...
    var globalEmitter = {
        ...
        fireEvent: function() {
            ...
            if (window.globalMgr && window.globalMgr.eventGlobalBus) {
                window.globalMgr.eventGlobalBus.trigger({
                    eventName: ename,
                    params: arguments
                })
            }
    ...
    var parseResponse = function(xhr, conf) {
        ...
        if (status == 0) {
            ...
            if (conf && conf.headers) {
                console.log("xhr-error", conf.headers["X-Service-Method"])
            }
        }
        return result
    };
        ...
        if (!conf.url) {
            if (conf.params && conf.params.hasOwnProperty("useAES")) {
                if ($sys.isWeixin()) {
                    ...
                    var appid = $sys.getAliAppid() || getUrlParam("appKey");
                    window.Logger && Logger.setReportParams({
                        appid: appid
                    });
    ...
    var ajaxErrorLog = function(logMsg) {
        if (window.Sentry) {
            var message = logMsg.url || "getway error";
            if (logMsg.headers && (logMsg.headers["X-Service-Id"] || logMsg.headers["X-Service-Method"])) {
                message = logMsg.headers["X-Service-Id"] + " method: " + logMsg.headers["X-Service-Method"]
            }
            Sentry.addBreadcrumb({
                message: message,
                type: "http"
            });
            Sentry.captureException(new Error("请求错误： " + JSON.stringify(logMsg)))
        }
    };
    var isRetry = false;
    var ajax = function(conf) {
        ...
        try {
            ...
            if (env.isZLB && url && method == "POST") {
                if ($env.zlbtoken) {
                    headers["X-Access-Token"] = $env.zlbtoken
                }
                if ($env.zlbclientId) {
                    headers["X-Client-Id"] = Number($env.zlbclientId)
                }
            }
            if ($env.getUrlParam("os") === "WX_THIRD" && url && url.indexOf("gateway") > 0) {
                if ($env.wxToken) {
                    headers["X-Access-Token"] = $env.wxToken
                }
                if ($env.wxClientId) {
                    headers["X-Client-Id"] = Number($env.wxClientId)
                }
            }
            if (env.ngariMiniAppForWeb && url && url.indexOf("gateway") > 0) {
                var appkey = $env.getUrlParam("appKey") || $sys.getWxAppid();
                var tk = env.getUrlParam("token") || $env.getCookie(appkey + "_" + "tk");
                var clientId = env.getUrlParam("clientId") || $env.getCookie(appkey + "_" + "client-id");
                var version = $env.getUrlParam("version");
                if (tk) {
            ...
            if (env.ngariWeApp && url && url.indexOf("gateway") > 0) {}
            if (headers) {
    ...
    var resolvePublicPath = function(res) {
        if (res.indexOf("http") >= 0) {
            return res
        }
        return env.resourcesHome + "/" + res
    };
    var manifest = function(cls) {
        if (is.String(cls)) {
            cls = resolveLocalClsName(cls);
            if ($env.manifest == null || cls.indexOf("??") > 0 || cls.indexOf("http") >= 0) {
                return cls
            }
    ...
    g.$loadHts = loadHts;
    var getAbsolutResPath = function(url) {
        if (url.indexOf("http") >= 0) {
            return url
        }
        return $env.cdnUrl(manifest(url))
    };
    g.$absolutResPath = getAbsolutResPath;
    ...
    $env.errorCatch = function(e) {
        ...
        window.Sentry ? Sentry.captureException(new Error(e)) : console.error($env.vconsole ? error : e);
    ...
    ;
    $env.getUrlDataFN = function(urlStr) {
        const urlObj = {};
        if (urlStr.indexOf("?") === -1)
            return null;
    ...
    ;
    env.initLogger = function() {
        if (env.wxDebug || env.vconsole) {
            return
        }
        Logger && Logger.useDefaults({
            ...
            fetchError: function(info, cb) {
                if (info && info.envInfo) {
                    try {
                        var envInfo = JSON.parse(info.envInfo);
                        if (envInfo.env == "premaster" || envInfo.env == "master") {
                            $ajax({
                                headers: {
    ...
    ;
    g.$sentryInit = function() {
        if (env.wxDebug || env.vconsole) {
            return
        }
        $require("lib.sentry-5-27-2").then(function() {
            var appid = $sys.getLocationAppId();
            appid = appid.length && appid[0];
            var dsn = "https://20bf6ed3c5754a3e9c4d0778bb0c483b@sentry.ngarihealth.com/2";
            if (window.Sentry) {
                Sentry.init({
                    dsn: dsn,
                    ...
                    sampleRate: .1,
                    tracesSampleRate: .1,
                    environment: appid || $env.appkey,
                    ignoreErrors: ["Non-Error promise rejection captured", "UnhandledRejection", "querySelectorAll", "TypeError: Cannot read property 'onMessage' of undefined", "Error: [$rootScope:inprog]", "Error: [$sce:itype]"]
                });
                $emitter.on("onToken", function(re, json, then) {
                    var urt = json.body;
                    if (urt) {
                        Sentry.setUser({
                            id: urt.userId,
                            username: urt.userName
                        })
                    }
                })
            }
