...
function getPickerLabel(id, value) {
  const option = dwlist[id];
  let result = "";
...
//日期类型转换为字符串 yyyyMMdd
function dateToString(e = new Date()) {
  return (
    "" + e.getFullYear() + addZero(e.getMonth() + 1) + addZero(e.getDate())
  );
}
...
function getAge(agestr) {
  ...
  let birthYear = agestr.substr(0, 4);
  let birthMonth = agestr.substr(4, 2);
  let birthDay = agestr.substr(6, 2);

  let nowDate = dateToString(new Date());
  let nowYear = nowDate.substr(0, 4);
  let nowMonth = nowDate.substr(4, 2);
  let nowDay = nowDate.substr(6, 2);
  if (nowDate < agestr) {
    return -1;
  }
...
function getMonthAge(beginStr, endStr) {
  var reg = new RegExp(
 ...
 * 套餐适用性别
 */
function getGenderFitted(gender) {
  let status = {};
  ...
  switch (gender) {
    case "F":
      status.title = "女性";
      status.color = "#F56C6C";
      status.backgroundColor = "#FAE8E8";
      return status;
    case "M":
      status.title = "男性";
      status.color = "#3394FF";
      status.backgroundColor = "#E2F0FF";
      return status;
    default:
      status.title = "通用";
      status.color = "#4F9C29";
      status.backgroundColor = "#E8FADF";
      return status;
  }
}
/**
 * 支付状态
 */
function getBillSatus(status) {
  let billStatus = {};

  switch (status) {
    case "0":
      billStatus.title = "未支付";
      billStatus.color = "#F56C6C";
      billStatus.backgroundColor = "#FAE8E8";
      return billStatus;
    case "1":
      billStatus.title = "支付处理中";
      billStatus.color = "#1A8FE6";
      billStatus.backgroundColor = "#E8F5FF";
      return billStatus;
    case "2":
      billStatus.title = "已支付";
      billStatus.color = "#4F9C29";
      billStatus.backgroundColor = "#E8FADF";
      return billStatus;
    case "3":
      billStatus.title = "退费处理中";
      billStatus.color = "#E57B39";
      billStatus.backgroundColor = "#FFF3E0";
      return billStatus;
    case "4":
      billStatus.title = "已退费";
      billStatus.color = "#757982";
      billStatus.backgroundColor = "#EBEEF5";
    case "5":
      billStatus.title = "已取消";
      billStatus.color = "#757982";
      billStatus.backgroundColor = "#EBEEF5";
      return billStatus;
    case "6":
      billStatus.title = "未知状态";
      billStatus.color = "#E79700";
      billStatus.backgroundColor = "#FBF5CC";
      return billStatus;
    case "7":
      billStatus.title = "支付失败";
      billStatus.color = "#E79700";
      billStatus.backgroundColor = "#FBF5CC";
      return billStatus;
    case "8":
      billStatus.title = "退费失败";
      billStatus.color = "#E79700";
      billStatus.backgroundColor = "#FBF5CC";
      return billStatus;
    case "9":
      billStatus.title = "支付异常，需要人工干预";
      billStatus.color = "#E79700";
      billStatus.backgroundColor = "#FBF5CC";
      return billStatus;
    case "91":
      billStatus.title = "支付对账异常，需要人工干预";
      billStatus.color = "#E79700";
      billStatus.backgroundColor = "#FBF5CC";
      return billStatus;
    case "10":
      billStatus.title = "退费异常，需要人工干预";
      billStatus.color = "#E79700";
      billStatus.backgroundColor = "#FBF5CC";
      return billStatus;
    case "11":
      billStatus.title = "交易关闭";
      billStatus.color = "#757982";
      billStatus.backgroundColor = "#EBEEF5";
      return billStatus;
    case "12":
      billStatus.title = "有退款";
      billStatus.color = "#E79700";
      billStatus.backgroundColor = "#FBF5CC";
      return billStatus;
    case "13":
      billStatus.title = "预约成功";
      billStatus.color = "#4F9C29";
      billStatus.backgroundColor = "#E8FADF";
      return billStatus;
    default:
 ...
 * @returns {boolean}
 */
function checkReferenceCode(referenceCode) {
  if (!referenceCode || referenceCode.length > 10) {
    return false;
  }
  var reg = new RegExp(
    /^[a-zA-Z0-9]*([a-zA-Z][0-9]|[0-9][a-zA-Z])[a-zA-Z0-9]*$/
  );
  return reg.test(referenceCode);
}
...
function GetCardBirthday(card) {
  ...
  if (card.length == 18) {
    birthdayno = card.substring(6, 14);
  } else if (card.length == 15) {
    birthdaytemp = card.substring(6, 12);
...
function GetProvince(card) {
  return id_card_area.getAreaByIdCard(card);
}
 ...
 * @constructor
 */
function GetCountyCode(card) {
  return id_card_area.getCountyCodeByIdCard(card);
}
 ...
 */
var myToast = "";
$().toastmessage({ sticky: false, position: "middle-bottom" });
...
function showToast(content) {
  myToast = $().toastmessage("showNoticeToast", content);
}

function checkGATId(papersNo) {
  //港澳校验
  /*H: 香港
    M: 澳门*/
  // if (papersNo.length == 11) {
  //     var reg = /^[H|M|C|W]([A-Z0-9])(\d{7,9})$/;
  var reg = /^([H|M](\d{9}))|([C|W]([A-Z0-9])(\d{7}))$/;
  return reg.test(papersNo);
  // }
  // else return false;
}
/**
...
var id_card_area = {
  ...
  getCountyCodeByIdCard: function (idCard) {
    if (!idCard || idCard.length < 6) return "";
...
function formatCurrency(num, precision = 2) {
  if (num || num == 0) {
    // 判断是否为数字
    if (!isNaN(Number(num)) && isFinite(num)) {
      let parts;
      num = Number(num);
      // 处理小数点位数
