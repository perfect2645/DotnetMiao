 ...
 * 压缩路径 https://tool.oschina.net/jscompress/
 * =====================================================
 */
/* global $:true */
...
+(function ($) {
  ...
  var defaults = {
    ...
    pushAnimationDuration: 400, //不要动这个400，这是解决安卓 animationEnd 事件无法触发的bug
  };
    ...
    if (typeof value === "undefined") {
      ...
      if (this[0] && this[0].getAttribute) {
        ...
        if (dataKey) {
          ...
          this[0].smElementDataStorage &&
          key in this[0].smElementDataStorage
        ) {
...
(function ($) {
  ...
  FastClick.prototype.onTouchStart = function (event) {
    ...
    if (event.timeStamp - this.lastClickTime < this.tapDelay) {
      var t1 = $(targetElement).attr("class");
      if (t1) {
        if (t1.indexOf("layui") == -1) {
          event.preventDefault();
        }
      }
    }
...
+(function ($) {
  ...
  $.modal = function (params) {
    ...
    if (defaults.modalTemplate) {
      if (!$._compiledTemplates.modal)
        $._compiledTemplates.modal = t7.compile(defaults.modalTemplate);
  ...
  $.modalLogin = function (text, title, callbackOk, callbackCancel) {
    ...
    return $.modal({
      ...
      afterText:
        ...
        defaults.modalUsernamePlaceholder +
  ...
  $.actions = function (target, params) {
    ...
    if (toPopover) {
      var actionsToPopoverTemplate =
        defaults.modalActionsToPopoverTemplate ||
      ...
      if (!$._compiledTemplates.actionsToPopover) {
        $._compiledTemplates.actionsToPopover = t7.compile(
      ...
      if (defaults.modalActionsTemplate) {
        if (!$._compiledTemplates.actions)
          $._compiledTemplates.actions = t7.compile(
            defaults.modalActionsTemplate
          );
    ...
    function sizePopover() {
      ...
      if (modalPosition === "top" || modalPosition === "bottom") {
        ...
        modalAngleLeft = Math.max(
          Math.min(modalAngleLeft, modalWidth - modalAngleSize * 2 - 6),
          6
        );
        ...
        modalAngleTop = Math.max(
          Math.min(modalAngleTop, modalHeight - modalAngleSize * 2 - 6),
          6
        );
  ...
  $.toast = function (msg, time) {
    var $toast = $("<div class='modal toast'>" + msg + "</div>").appendTo(
      document.body
    );
    $.openModal($toast);
    setTimeout(function () {
  ...
  $.openModal = function (modal) {
    ...
    if (isToast) {
      ...
      modal.css({
        marginLeft:
          -Math.round(parseInt(window.getComputedStyle(modal[0]).width) / 2) +
          "px", //
      });
    }
  ...
  $.closeModal = function (modal) {
    ...
    modal.remove(); //CSQ 新增，修复 弹窗无法正常关闭，强制关闭，也就没有了关闭后的动画
    // Picker modal body class
    if (isPickerModal) {
  ...
  function handleClicks(e) {
    ...
    if (clicked.hasClass("modal-overlay")) {
      ...
      if (
        ...
        defaults.actionsCloseByOutside
      )
  ...
  $(function () {
    $(document).on(
      "click",
      " .modal-overlay, .popup-overlay, .close-popup, .open-popup, .open-popover, .close-popover, .close-picker",
      handleClicks
    );
  ...
  var Calendar = function (params) {
    ...
    p.initCalendarEvents = function () {
      ...
      function handleDayClick(e) {
        ...
        if (day.hasClass("picker-calendar-day-selected") && !p.params.multiple)
          return;
    ...
    p.layout = function () {
      ...
      if (p.params.weekHeader) {
        ...
        weekHeaderHTML =
          '<div class="picker-calendar-week-days">' + weekHeaderHTML + "</div>";
      }
  ...
  var Picker = function (params) {
    ...
    p.initPickerCol = function (colElement, updateItems) {
      ...
      col.updateItems = function (
        ...
        var percentage =
          (translate -
            (Math.floor((translate - maxTranslate) / itemHeight) * itemHeight +
              maxTranslate)) /
          itemHeight;
    ...
    return this.each(function () {
      ...
      if (!picker) {
        ...
        if (params.value === undefined && inputValue !== "") {
          params.value =
            params.cols.length > 1 ? inputValue.split(" ") : [inputValue];
        }
    ...
    return this.each(function () {
      ...
      var defaults = {
        ...
        onChange: function (picker, values, displayValues) {
          var days = getDaysByMonthAndYear(
            picker.cols[1].value,
            picker.cols[0].value
          );
...
+(function ($) {
  ...
  $.initPullToRefresh = function (pageContainer) {
    ...
    function handleTouchMove(e) {
      ...
      if (!isMoved) {
        ...
        if (dynamicTriggerDistance) {
          ...
          if (triggerDistance.indexOf("%") >= 0)
            triggerDistance =
              (container[0].offsetHeight * parseInt(triggerDistance, 10)) / 100;
        }
      ...
      if ((touchesDiff > 0 && scrollTop <= 0) || scrollTop < 0) {
        ...
        if (
          $.os.ios &&
          parseInt($.os.version.split(".")[0], 10) > 7 &&
          scrollTop === 0 &&
          !wasScrolled
        )
          useTranslate = true;
  ...
  $.destroyPullToRefresh = function (pageContainer) {
    ...
    var pullToRefreshContent = pageContainer.hasClass("pull-to-refresh-content")
      ? pageContainer
      : pageContainer.find(".pull-to-refresh-content");
    if (pullToRefreshContent.length === 0) return;
    if (pullToRefreshContent[0].destroyPullToRefresh)
      pullToRefreshContent[0].destroyPullToRefresh();
  };
})($); //jshint ignore:line
...
+(function ($) {
  ...
  $.initInfiniteScroll = function (pageContainer) {
    pageContainer = $(pageContainer);
    var infiniteContent = pageContainer.hasClass("infinite-scroll")
      ? pageContainer
      : pageContainer.find(".infinite-scroll");
...
+(function ($) {
  ...
  Index.prototype.draw = function () {
    ...
    var titleTexts = this.titles
      .map(function (i, t) {
        return $(t).data("index") || $(t).text();
      })
      .toArray();

    this.indexList = $("<ul class='index-list-bar'></ul>").appendTo(
      this.list.parents(".page")
    );
    this.indexList.html(this.tpl({ indexes: titleTexts }));
    this.indexList.on($.touchEvents.start, $.proxy(this.touchStart, this));
    this.indexList.on(
      $.touchEvents.start + " " + $.touchEvents.move,
      $.proxy(this.touchMove, this)
    );
  ...
  Index.prototype.getElementOnTouch = function (position) {
    ...
    this.indexList.find("li").each(function () {
      ...
      if (
        ...
        offset.top + $this.outerHeight() > position.y
      ) {
        result = $this;
      }
    });
  ...
  $.fn.indexList.prototype.defaults = {
    ...
    indexListTemplate:
      "{{#indexes}}<li data-index={{@index}}><strong>{{this}}</strong></li>{{/indexes}}",
  };
...
+(function ($) {
  ...
  $.openPanel = function (panel) {
    ...
    var transitionEndTarget =
      effect === "reveal" ? $($.getCurrentPage()) : panel;
  ...
  $.initSwipePanels = function () {
    ...
    function handleTouchStart(e) {
      ...
      if (
        !$.allowPanelOpen ||
        (!swipePanel && !swipePanelOnlyClose) ||
        isTouched
      )
        return;
...
+(function ($) {
  ...
  var Router = function () {
    ...
    this.state.setItem(
      "stateid",
      parseInt(this.state.getItem("stateid") || 1) + 1
    );
  ...
  Router.prototype.init = function () {
    ...
    if (!currentPage[0])
      currentPage = $(".page").eq(0).addClass("page-current");
    var hash = location.hash;
    if (currentPage[0] && !currentPage[0].id)
      currentPage[0].id = hash ? hash.slice(1) : this.genRandomID();
  ...
  Router.prototype.animatePages = function (
    leftPage,
    rightPage,
    leftToRight,
    noTransition
    ...
    var removeClasses =
      "page-left page-right page-from-center-to-left page-from-center-to-right page-from-right-to-center page-from-left-to-center";
    if (noTransition) {
      if (!leftToRight) {
      ...
      if (!leftToRight) {
        ...
        leftPage
          ...
          .removeClass("page-current"); //csq 强制动画显示
        rightPage
          .removeClass(removeClasses)
          .addClass("page-from-right-to-center page-current"); //csq  强制动画显示
        setTimeout(function () {
          leftPage.removeClass(removeClasses);
        }, 400); //CSQ 新增，修复 左边侧滑时无法正常隐藏导致画面叠加
      } else {
        leftPage.trigger("pageAnimationStart", [rightPage[0].id, rightPage]);
        rightPage.removeClass(removeClasses).removeClass("page-current");
        leftPage.removeClass(removeClasses).addClass("page-current");
        ...
        leftPage
          .removeClass(removeClasses)
          .addClass("page-from-left-to-center page-current"); //csq 强制动画显示
        setTimeout(function () {
      ...
      /*
       ...
       //   
          ...
          setTimeout(function () {leftPage.removeClass(removeClasses);}, 300);//CSQ 新增，修复 左边侧滑时无法正常隐藏导致画面叠加
  ...
  $(function () {
    ...
    $(document).on("click", "a", function (e) {
      ...
      //router.loadPage(url, noAnimation, replace, reload)

      if (!url || url === "#" || /javascript:.*;/.test(url)) return;

      router.loadPage(
        url,
        $target.hasClass("no-transition") ? true : undefined,
        $target.hasClass("replace") ? true : undefined
      ); //undefined is different to false
    });
  });
})($);
// jshint ignore: end
