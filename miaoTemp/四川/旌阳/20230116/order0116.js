...
(function () {
  ...
  FastClick.prototype.focus = function (targetElement) {
    ...
    if (
      ...
      unsupportedType.indexOf(targetElement.type) === -1
    ) {
...
+(function ($) {
  ...
  $.modal = function (params) {
    ...
    var noButtons =
      !params.buttons || params.buttons.length === 0 ? "modal-no-buttons" : "";
  ...
  $.toast = function (msg, duration, extraclass) {
    var $toast = $(
      '<div class="modal toast ' + (extraclass || "") + '">' + msg + "</div>"
...
+(function (window) {
  ...
  var utils = (function () {
    ...
    me.isBadAndroid =
      ...
      false; //this will cause many android device scroll flash; so set it to false!
...
+(function ($) {
  ...
  var Scroller = function (pageContent, _options) {
    ...
    var useJSScroller =
      ...
      (type === "auto" &&
        $.device.android &&
        $.compareVersion("4.4.0", $.device.osVersion) > -1) ||
      (type === "auto" &&
        $.device.ios &&
        $.compareVersion("6.0.0", $.device.osVersion) > -1);
    ...
    if (useJSScroller) {
      ...
      if (!$pageContentInner[0]) {
        ...
        if (children.length < 1) {
          ...
          $pageContent.html(
            '<div class="content-inner">' + $pageContent.html() + "</div>"
          );
        }
      }
      ...
      if ($pageContent.hasClass("pull-to-refresh-content")) {
        ...
        var minHeight =
          $(window).height() + ($pageContent.prev().hasClass(".bar") ? 1 : 61);
  ...
  $.getScroller = function (content) {
    ...
    content = content.hasClass("content")
      ? content
...
+(function ($) {
  ...
  var initPullToRefreshJS = function (pageContainer) {
    ...
    var page = eventsTarget.hasClass("content")
      ? eventsTarget
      : eventsTarget.parents(".content");
...
+(function ($) {
   ...
   * @private
   */
  Router.prototype._switchToDocument = function (
    url,
    ignoreCache,
    isPushState,
    direction
  ...
  Router.prototype._doSwitchDocument = function (url, isPushState, direction) {
    ...
    $currentSection.trigger(EVENTS.beforePageSwitch, [
  ...
  Router.prototype._parseDocument = function (url, $doc) {
    ...
    if (!$innerView.length) {
      throw new Error(
        "missing router view mark: " + routerConfig.sectionGroupClass
      );
    }
   ...
   * @private
   */
  Router.prototype._animateDocument = function (
    $from,
    $to,
    $visibleSection,
    direction
    ...
    $visibleSectionInFrom
      .addClass(routerConfig.visiblePageClass)
      .removeClass(routerConfig.curPageClass);
    $visibleSection.trigger(EVENTS.pageAnimationStart, [
      sectionId,
      $visibleSection,
    ]);
    ...
    if (direction === "none") {
      ...
      $visibleSection.trigger(EVENTS.pageAnimationEnd, [
        sectionId,
        $visibleSection,
      ]);
  ...
  Router.prototype._animateElement = function ($from, $to, direction) {
    ...
    if (direction === "none") {
      $to
        .find("." + routerConfig.curPageClass)
  ...
  Router.prototype._back = function (state, fromState) {
    ...
    if (this._isTheSameDocument(state.url.full, fromState.url.full)) {
      ...
      switch (
        this.$view.find("." + routerConfig.curPageClass).attr("data-direction")
      ) {
        case "none":
          direction = "none";
          break;
...
+(function ($) {
  ...
  $.lastPosition = function (options) {
    ...
    $(window)
      ...
      .on("beforePageSwitch", function (event, id, arg) {
        updateMemory(id, arg);
      });
    $(window)
      .off("pageAnimationStart")
      .on("pageAnimationStart", function (event, id, arg) {
        getMemory(id, arg);
      });
    //让后退页面回到之前的高度
