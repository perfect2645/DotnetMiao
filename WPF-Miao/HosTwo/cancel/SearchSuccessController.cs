...
namespace HosTwo.cancel
{
    internal class SearchSuccessController : HttpClientBase
    {
        ...
        public Task SearchHistoryAsync(HosTwoLogin user)
        {
            return Task.Factory.StartNew(() => SearchHistory(user));
        }

        private void SearchHistory(HosTwoLogin user)
        {
            try
            {
                var content = new HosTwoContent("", user);
        ...
        private void SaveHistory(JsonElement data, HosTwoLogin user)
        {
