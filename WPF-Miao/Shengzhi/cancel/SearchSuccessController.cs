...
namespace Shengzhi.cancel
{
    internal class SearchSuccessController : HttpClientBase
    {
        ...
        public Task SearchHistoryAsync(ShengzhiLogin user)
        {
            return Task.Factory.StartNew(() => SearchHistory(user));
        }

        private void SearchHistory(ShengzhiLogin user)
        {
            try
            {
                //可以通过设置offset参数过滤
        ...
        private void SaveHistory(JsonElement data, ShengzhiLogin user)
        {
