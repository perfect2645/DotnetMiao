...
namespace B114.login
{
    internal class TokenController : HttpClientBase
    {
        ...
        public Task<bool> GetTokenAsync(B114Login user)
        {
            return Task.Factory.StartNew(() => GetToken(user));
        }

        private bool GetToken(B114Login user)
        {
            try
            {
        ...
        private bool CheckSaveToken(Dictionary<string, object> headers, B114Login user)
        {
            if (!headers.HasItem())
            {
