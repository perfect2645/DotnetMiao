...
namespace Jian.cancel
{
    internal class SearchSuccessController : HttpClientBase
    {
        ...
        public Task SearchHistoryAsync(JianLogin user)
        {
            return Task.Factory.StartNew(() => SearchHistory(user));
        }

        private void SearchHistory(JianLogin user)
        {
            try
            {
                var content = new JianContent("", user);
        ...
        private void SaveHistory(JsonElement data, JianLogin user)
        {
