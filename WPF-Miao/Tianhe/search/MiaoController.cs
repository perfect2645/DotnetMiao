...
namespace Tianhe.search
{
    internal class MiaoController : HttpClientBase
    {
        ...
        public void SearchMiaoAsync(string date, TianheLogin user)
        {
            Date = date;
            Task.Factory.StartNew(() => SearchMiao(date, user));
        }

        public bool SearchMiao(string date, TianheLogin user)
        {
            Date = date;
            try
            {
                ...
                var url = $"ldrmyy120.com/rest/v1/api/examine/shift_time_for_vaccine/{deptId}/?date={date}&hospital={hosId}";
        ...
        private bool CheckTime(JsonElement data, TianheLogin user)
        {
