...
namespace Dalian.cancel
{
    internal class SearchSuccessController : HttpClientBase
    {
        ...
        public Task SearchHistoryAsync(DalianLogin user)
        {
            return Task.Factory.StartNew(() => SearchHistory(user));
        }

        private void SearchHistory(DalianLogin user)
        {
            try
            {
                var content = new DalianContent("", user);
        ...
        private void SaveHistory(JsonElement data, DalianLogin user)
        {
