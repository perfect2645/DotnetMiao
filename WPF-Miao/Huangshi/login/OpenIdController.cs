...
namespace Huangshi.login
{
    internal class OpenIdController : HttpClientBase
    {
        ...
        public async Task GetOpenIdAsync(HuangshiLogin user)
        {
            await Task.Factory.StartNew(() => GetOpenId(user));
        }

        private void GetOpenId(HuangshiLogin user)
        {
            try
            {
        ...
        private void SaveOpenId(JsonElement data, HuangshiLogin user)
        {
