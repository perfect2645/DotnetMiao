...
namespace Lzy.cancel
{
    internal class SearchSuccessController : HttpClientBase
    {
        ...
        public Task SearchHistoryAsync(LzyLogin user)
        {
            return Task.Factory.StartNew(() => SearchHistory(user));
        }

        private void SearchHistory(LzyLogin user)
        {
            try
            {
                var content = new LzyContent("", user);
        ...
        private void SaveHistory(JsonElement data, LzyLogin user)
        {
