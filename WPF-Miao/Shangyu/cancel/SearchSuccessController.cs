...
namespace Shangyu.cancel
{
    internal class SearchSuccessController : HttpClientBase
    {
        ...
        public Task SearchHistoryAsync(ShangyuLogin user)
        {
            return Task.Factory.StartNew(() => SearchHistory(user));
        }

        private void SearchHistory(ShangyuLogin user)
        {
            try
            {
                var content = new ShangyuContent("", user);
        ...
        private void SaveHistory(JsonElement data, ShangyuLogin user)
        {
