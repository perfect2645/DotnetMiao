...
namespace Dxm.cancel
{
    internal class SearchSuccessController : HttpClientBase
    {
        ...
        public Task SearchHistoryAsync(DxmLogin user)
        {
            return Task.Factory.StartNew(() => SearchHistory(user));
        }

        private void SearchHistory(DxmLogin user)
        {
            try
            {
                var content = new DxmContent("", user);
        ...
        private void SaveHistory(JsonElement data, DxmLogin user)
        {
