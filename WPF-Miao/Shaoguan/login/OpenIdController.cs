...
namespace Shaoguan.login
{
    internal class OpenIdController : HttpClientBase
    {
        ...
        public async Task GetOpenIdAsync(ShaoguanLogin user)
        {
            await Task.Factory.StartNew(() => GetOpenId(user));
        }

        private void GetOpenId(ShaoguanLogin user)
        {
            try
            {
        ...
        private void SaveOpenId(JsonElement data, ShaoguanLogin user)
        {
