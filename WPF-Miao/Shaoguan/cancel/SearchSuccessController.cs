...
namespace Shaoguan.cancel
{
    internal class SearchSuccessController : HttpClientBase
    {
        ...
        public Task SearchHistoryAsync(ShaoguanLogin user)
        {
            return Task.Factory.StartNew(() => SearchHistory(user));
        }

        private void SearchHistory(ShaoguanLogin user)
        {
            try
            {
        ...
        private void SaveHistory(JsonElement data, ShaoguanLogin user)
        {
