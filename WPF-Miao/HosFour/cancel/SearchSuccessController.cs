...
namespace HosFour.cancel
{
    internal class SearchSuccessController : HttpClientBase
    {
        ...
        public Task SearchHistoryAsync(HosFourLogin user)
        {
            return Task.Factory.StartNew(() => SearchHistory(user));
        }

        private void SearchHistory(HosFourLogin user)
        {
            try
            {
        ...
        private void SaveHistory(JsonElement data, HosFourLogin user)
        {
