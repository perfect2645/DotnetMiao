...
{
    internal class ShishiViewModel : OnTimeViewModel
    {
        #region Properties
        ...
        public ShishiViewModel(LogPanel logPanel) : base(logPanel)
        {
            InitCommands();
            InitStaticData();
        ...
        {
            MainSession.Users = FileReader.DeserializeFile<List<ShishiLogin>>("Login.json");
        ...
        private Order BuildOneOrder(ShishiLogin user, string date, string timeId)
        {
        ...
        private async Task ExecuteSearchHistory()
        {
            ...
            {
                var defaultUser = MainSession.Users.FirstOrDefault();
                var historyController = HttpServiceController.GetService<HistoryController>();
        ...
        {
            var selectedDept = SelectedDepartment as ShishiHospital;
