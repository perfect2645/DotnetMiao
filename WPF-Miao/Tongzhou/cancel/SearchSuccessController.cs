...
namespace Tongzhou.cancel
{
    internal class SearchSuccessController : HttpClientBase
    {
        ...
        public Task SearchHistoryAsync(TongzhouLogin user)
        {
            return Task.Factory.StartNew(() => SearchHistory(user));
        }

        private void SearchHistory(TongzhouLogin user)
        {
            try
            {
                var content = new TongzhouContent("", user);
        ...
        private void SaveHistory(JsonElement data, TongzhouLogin user)
        {
