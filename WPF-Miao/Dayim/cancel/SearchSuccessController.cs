...
namespace Dayim.cancel
{
    internal class SearchSuccessController : HttpClientBase
    {
        ...
        public Task SearchHistoryAsync(DayimLogin user)
        {
            return Task.Factory.StartNew(() => SearchHistory(user));
        }

        private void SearchHistory(DayimLogin user)
        {
            try
            {
                var content = new DayimContent("", user);
        ...
        private void SaveHistory(JsonElement data, DayimLogin user)
        {
