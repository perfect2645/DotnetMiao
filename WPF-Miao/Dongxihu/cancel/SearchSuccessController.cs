...
namespace Dongxihu.cancel
{
    internal class SearchSuccessController : HttpClientBase
    {
        ...
        public Task SearchHistoryAsync(DongxihuLogin user)
        {
            return Task.Factory.StartNew(() => SearchHistory(user));
        }

        private void SearchHistory(DongxihuLogin user)
        {
            try
            {
                var content = new DongxihuContent("", user);
        ...
        private void SaveHistory(JsonElement data, DongxihuLogin user)
        {
